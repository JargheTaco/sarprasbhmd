...
import { AlertCircle, CheckCircle2, Plus, ShieldCheck, Trash2, UserPlus, Users } from 'lucide-react';
...
export default function AkunPage() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg(`Akun @${data.user.username} berhasil dibuat.`);
      await fetchAccounts();
    } catch (error) {
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.45fr)] gap-6 items-start">
        <section className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-5">
            ...
            <div><h2 className="font-bold text-slate-900 text-sm">Buat Akun Baru</h2><p className="text-[11px] text-slate-500">Nama tampilan dapat dibuat sesuai kebutuhan.</p></div>
          </div>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <label className="block space-y-1.5"><span className="text-xs font-bold text-slate-700">Password</span><input required minLength={6} type="password" value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })} placeholder="Minimal 6 karakter" className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" /></label>
            ...
            <button disabled={submitting} className="w-full py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer"><Plus className="w-4 h-4" />{submitting ? 'Menyimpan...' : 'Buat Akun'}</button>
          </form>
        </section>
