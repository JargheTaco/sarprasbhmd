      ...
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.45fr)] gap-6 items-start">
        <section className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <label className="block space-y-1.5"><span className="text-xs font-bold text-slate-700">Nama lengkap</span><input required value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="Contoh: Budi Santoso" className="w-full bg-white text-slate-900 placeholder:text-slate-400 px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" /></label>
            <label className="block space-y-1.5"><span className="text-xs font-bold text-slate-700">Username</span><input required value={formData.username} onChange={(e) => setFormData({ ...formData, username: e.target.value })} placeholder="budi.santoso" className="w-full bg-white text-slate-900 placeholder:text-slate-400 px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" /></label>
            <label className="block space-y-1.5"><span className="text-xs font-bold text-slate-700">Password {editingAccount && <span className="font-normal text-slate-400">(kosongkan jika tidak diubah)</span>}</span><input required={!editingAccount} minLength={6} type="password" value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })} placeholder={editingAccount ? 'Password baru (opsional)' : 'Minimal 6 karakter'} className="w-full bg-white text-slate-900 placeholder:text-slate-400 px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" /></label>
            <label className="block space-y-1.5"><span className="text-xs font-bold text-slate-700">Peran akun</span><select value={formData.role} onChange={(e) => setFormData({ ...formData, role: e.target.value as UserRole })} className="w-full bg-white text-slate-900 px-3.5 py-2.5 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"><option value="STAFF_SARPRAS">Staff Sarpras</option><option value="KEPALA_SARPRAS">Kepala Sarpras</option><option value="KEPALA_ADMIN_UMUM">Kepala Administrasi Umum</option><option value="ADMIN">Administrator</option></select></label>
